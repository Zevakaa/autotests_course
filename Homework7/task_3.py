...
class PublicTransport:
    """ Класс для работы с публичным транспортом """

    def __init__(self, transport_brand, transport_engine_power, transport_year, transport_color,
                 transport_max_speed) -> None:
        self.brand = transport_brand
        self._engine_power = transport_engine_power
        self.year = transport_year
        self.color = transport_color
        self.max_speed = transport_max_speed
...
class Bus(PublicTransport):
    """ Класс для работы с автобусами. Наследует от публичного траспорта """

    def __init__(self, bus_brand, bus_engine_power, bus_year, bus_color, bus_max_speed, bus_passengers,
                 bus_park, bus_fare) -> None:
        super().__init__(bus_brand, bus_engine_power, bus_year, bus_color, bus_max_speed)
        self.passengers = bus_passengers
        self.__park = bus_park
        self._fare = bus_fare

    @property
    def park(self) -> int:
        """ Возврат значения park """
        return self.__park

    @park.setter
    def park(self, number):
        """ При присвоении проверка номера парка, что он в диапазоне от 1000 до 9999 """
        assert 1000 <= number <= 9999
        self.__park = number


class Tram(PublicTransport):
    """ Класс для работы с трамваями. Наследует от публичного траспорта """
    def __init__(self, tram_brand, tram_engine_power, tram_year, tram_color, tram_max_speed, tram_route,
                 tram_path, tram_fare) -> None:
        super().__init__(tram_brand, tram_engine_power, tram_year, tram_color, tram_max_speed)
        self.__route = tram_route
        self.path = tram_path
        self._fare = tram_fare
