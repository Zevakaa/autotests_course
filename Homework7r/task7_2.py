...
class PersonInfo:
    """ Работа со списком сотрудников. Преобразование ФИ в краткую форму, рассчет индексации ЗП """
    def __init__(self, employee_name, employee_age, *employee_department) -> None:
        """
        :name имя сотрудника:
        :age возраст:
    ...
    def path_deps(self) -> str:
        """ Возвращает строку с полным названием подразделения с разделителем ' --> ' """
        return ' --> '.join(self.department)

    def new_salary(self) -> int:
        """ Возвразает число ЗП, рассчитываемой по формуле: 1337 * возраст * суммарное кол-во вхождений трех наиболее
        часто встречающихся букв из списка подразделений"""
        dep_str = ''.join(self.department)
        letter_dict = {}
        for letter in dep_str:
            letter_dict[letter] = letter_dict.get(letter, 0) + 1
        top_letters = dict(sorted(letter_dict.items(), key=lambda item: item[1], reverse=True))
        return 1337 * self.age * sum(list(top_letters.values())[:3])
