...
class RomanNums:
    ...
    def from_roman(self) -> int:
        """ Возвращает арабоское число """
        roman_dict = {'M': 1000, 'CM': 900, 'D': 500, 'CD': 400, 'C': 100,
                      'XC': 90, 'L': 50, 'XL': 40, 'X': 10, 'IX': 9, 'V': 5, 'IV': 4, 'I': 1}
        arab_number = 0

        for idx in range(len(self.roman_number) - 1):
            if roman_dict[self.roman_number[idx]] < roman_dict[self.roman_number[idx + 1]]:
                arab_number -= roman_dict[self.roman_number[idx]]
            else:
                arab_number += roman_dict[self.roman_number[idx]]
        arab_number += roman_dict[self.roman_number[len(self.roman_number) - 1]]
        return arab_number
