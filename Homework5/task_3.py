...
def everything_for_your_cat(cats_data):
    """ Преобразование в строку списка кортежей "cats_data" с группировкой данных по владельцу"""
    ...
    for X in cats_data:
        # Получение уникальных записей о владельцах
        if (str(X[2] + " " + X[3]) in owners_data) is False:
            owners_data.append(str(X[2] + " " + X[3]))
            res_data.append(str(X[2] + " " + X[3] + ": " + X[0] + ", " + str(X[1])))
        # Обогащение данными записей владельцев нескольких котов
        else:
            for idx1 in range(len(res_data)):
                if str(X[2] + " " + X[3]) in res_data[idx1]:
                    res_data[idx1] = res_data[idx1] + "; " + X[0] + ", " + str(X[1])

    for idx2 in res_data:
        our_str += str(idx2 + "\n")
