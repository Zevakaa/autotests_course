...
def josephus_task(num_people, kill_num):
    death_list = [idx for idx in range(1, num_people + 1)]
    skip = kill_num - 1
    locator_index = skip % len(death_list)

    while len(death_list) > 1:
        del death_list[locator_index]
        locator_index = (locator_index + skip) % len(death_list)
    print(death_list)
    survivor = death_list[0]
